...
class HeadingDitem(BlockDitem):
	...
	for l in range(1,level-1):
            underliner += "=" 
        return underliner+text +underliner + '\n'
...
#        result = tablehrule(colwidths) + '\n'
        ...
        for tr in self.children:
            ...
            result = result + '\n' 
#            + tablehrule(colwidths, rule) 
            
        return result

class ParametersTableDitem(BlockDitem):
    def __init__(self, type):
        BlockDitem.__init__(self, type)
    def format(self, width):
        # Uses table rendering algorithm of w3m
...
def handleNode(node):
    if node.nodeType == node.TEXT_NODE and node.data.startswith(u'Parameter'):
        node.data = ''
        return handleTextSpecial(node)
    elif node.nodeType == node.TEXT_NODE:
        return handleText(node)
    elif node.nodeName=='div' and (node.getAttribute('class')=='related' or node.getAttribute('class')=='sphinxsidebar'):  # HARDWIRED
        return Ditem('')
    ...
    elif node.nodeName=='div' and node.getAttribute('class')=='footer':
        return Ditem('')
#    elif node.nodeName in ('th') and node.getAttribute('class')=='field-name' and node.childNodes[0].data :
#        node.childNodes[0].data = '\n' + node.childNodes[0].data + '\n'
##        node.nodeValue = '\n' + node.nodeValue + '\n'
#        return handleGenericBlock(node)
    elif node.nodeName in ('body', 'div', 'p', 'td', 'th'):
        return handleGenericBlock(node)
    elif node.nodeName in ('em') and node.parentNode.tagName == 'td':
        return handleEmphasisSpecial(node)
    elif node.nodeName in ('em', 'i'):
        return handleEmphasis(node)
    elif node.nodeName in ('strong', 'b'):
        return handleStrong(node)
    elif node.nodeName in ('tt'):
        return handleTT(node)
    elif node.nodeName in ('ol', 'ul', 'dl'):
        return handleList(node)
    elif node.nodeName in ('li', 'dd', 'dt'):
        return handleListItem(node)
    elif node.nodeName in ('tbody') and node.parentNode.getAttribute('class')=='docutils':
        return handleTable(node)
    elif node.nodeName in ('tbody'):
        return handleTable(node)
...
def handleTextSpecial(node):
#    print "NewLinesDitem " + node.data
    return NewLinesDitem(node.data)
...
def handleAnchor(node):
    ...
    result.text = result.text+ ' (' +target+ ')'
    return result
...
def handleEmphasis(node):
    ...
    result.type = node.nodeName
    if result.text:
        result.text = '_' + result.text + '_'
    return result

def handleEmphasisSpecial(node):
    result = mergeChildren(node)
    result.type = node.nodeName
    if result.text:
        result.text = '* _' + result.text + '_'
...
def handleTT(node):
    ...
    if result.text and node.getAttribute('class') == 'descname':
        if hasattr(node.previousSibling,'tagName') and node.previousSibling.tagName == 'tt':
#            raise Exception( node.previousSibling.tagName )
#            raise Exception(dir( node.previousSibling) )
            result.text = '*' + result.text + '*'
        else:
            result.text = '` ` *' + result.text + '*'
    if result.text and node.getAttribute('class') == 'descclassname':
        result.text = '`' + result.text + '`'
    return result
...
def handleParametersTable(node):
    result = ParametersTableDitem(node.nodeName)
    # Ignore table contents that are not tr
    result.children = [x
        for x in processChildren(node) if x.type=='tr']
    return result
...
all_ = ditem.format(195)
all_ = all_.replace(u'\xc2','').replace(u'\xb6','').replace('Returns:', 28*' ' +'Returns:').replace('Return type:', 20*' ' +'Return type:')
outf.write(all_ + '\n')
